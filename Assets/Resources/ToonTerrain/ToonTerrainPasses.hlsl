...
half SampleToonRamp(half ndotl)
{
    float2 rampUv = TRANSFORM_TEX(float2(ndotl, 0.5h), _ToonRamp);
    half rampSample = SAMPLE_TEXTURE2D(_ToonRamp, sampler_ToonRamp, rampUv).r;
    half edge = max(_RampSmoothness, 1e-4h);
    return smoothstep(_RampThreshold - edge, _RampThreshold + edge, rampSample);
}

half3 ApplyToonLighting(InputData inputData, half3 albedo)
{
    ...
    half toon = SampleToonRamp(ndotl);
    half attenuation = mainLight.distanceAttenuation * mainLight.shadowAttenuation;
    half shadow = lerp(_ShadowIntensity, 1.0h, attenuation);
    half3 lighting = mainLight.color * (toon * shadow);
    ...
    for (uint i = 0; i < lightCount; ++i)
    {
        ...
        half toonAdd = SampleToonRamp(ndotlAdd);
        half attenuationAdd = light.distanceAttenuation * light.shadowAttenuation;
        half shadowAdd = lerp(_ShadowIntensity, 1.0h, attenuationAdd);
        lighting += light.color * (toonAdd * shadowAdd);
    }
#endif

    half shininess = lerp(1.0h, 128.0h, _Shininess);
    half3 halfDir = normalize(mainLight.direction + viewDirWS);
    half spec = pow(saturate(dot(normalWS, halfDir)), shininess);
    half3 specular = _SpecularColor.rgb * spec;

    return albedo * (lighting + inputData.bakedGI) + specular;
}
...
#endif
{
    ...
    float3 bf = normalize(abs(inputData.normalWS));
    bf /= dot(bf, (float3) 1);

    // Triplanar mapping
    float2 tx = inputData.positionWS.zy;
    float2 ty = inputData.positionWS.zx;
    float2 tz = inputData.positionWS.xy;

    // Base color
    half4 cx = _VerticalTex.Sample(sampler_VerticalTex, TRANSFORM_TEX(tx, _VerticalTex)) * bf.x;
    half4 cy = _VerticalTex.Sample(sampler_VerticalTex, TRANSFORM_TEX(ty, _VerticalTex)) * bf.y;
    half4 cz = _VerticalTex.Sample(sampler_VerticalTex, TRANSFORM_TEX(tz, _VerticalTex)) * bf.z;
    half4 tri = (cx + cy + cz);
    
    if (dot(inputData.normalWS, half3(0, 1, 0)) <= _WallTextureCutoff)
    {
        albedo = tri;
    }
    ...
    half3 toonColor = ApplyToonLighting(inputData, albedo);
    half4 color = half4(toonColor, alpha);
    SplatmapFinalColor(color, inputData.fogCoord);
